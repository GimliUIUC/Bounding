...
th0 = x_opt(1);
bx = bezier(1,cox1,cox2);
x0 = L/2*cos(th0)-bx*stride/2;
...
book.t_TD_f = te;
Tair_prev = Xout(end,7);
Tst_prev = Xout(end,10);
Tst_new = Tst - k_time*(Tst_prev + Tair_prev - T/2);
Xout(end,10) = Tst_new;
s_sw_f = Xout(end,8);
s_sw_f(s_sw_f>1) = 1;
bx = bezier(s_sw_f,cox1,cox2);
% stride = control.dxd*Tst_new;

Xout(end,1) = -L/2*cos(X(end,3))-stride/2*bx;    % reset x coordinate
Xout(end,7) = 0;    % reset t_s
...
%%
% [GRF,tau] = animation_oneFrame(t_even,X_even,teout, Xraw_even, params,control,50);
